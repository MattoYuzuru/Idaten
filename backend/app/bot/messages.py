...
EXTERNAL_AI_CONSENT_TEXT = (
    "Текст, изображение или голос могут содержать описание пробежки, самочувствия, "
    "сна, усталости и боли. Текущее содержимое будет однократно передано внешнему "
    "OpenAI provider только для заполнения typed-черновика. История пробежек, цель, "
    "профиль, GPS и Telegram identity не передаются. Raw input и transcript не "
    "сохраняются; остаются только hash, служебный audit и подтверждённые вами поля. "
    "Рекомендация всегда рассчитывается локально детерминированными правилами."
)

HELP_TEXT = (
    "<b>Idaten 1.0</b>\n\n"
    "Сохраняет пробежки приватно, показывает личный прогресс и рассчитывает одну "
    "адаптивную следующую пробежку после readiness check-in. Выберите раздел."
)
...
/run открывает выбор способа добавления. /stats показывает текущую неделю, два 28-дневных окна и график. /pr отделяет фактические результаты от оценок по темпу. /next предлагает выбрать цель, подтвердить самочувствие и получить одну безопасно ограниченную следующую пробежку или отдых.""",
    "imports": """<b>Файлы</b>
...
/link создаёт одноразовый код для Android. /devices показывает подключения, а /revoke_device отзывает выбранное устройство. Пробежки и optional sleep summary синхронизируются только вручную в Android-приложении; отказ от READ_SLEEP ничего не блокирует.""",
