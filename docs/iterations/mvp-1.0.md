...
- `motivation: int | None` — 1..5, полностью optional;
...
`false`. `general_fatigue` и `illness_symptoms` обязательны в любом confirmed check-in.
`session_rpe` запрещён для PRE_RUN. Все sleep fields полностью optional;
`sleep_summary_id` не даёт права без подтверждения check-in автоматически использовать
Health Connect.
...
4. полностью optional сон: quality 1–5 и/или duration либо editable Health Connect
   prefill; шаг можно пропустить;
...
optional motivation/sleep нейтральны; отсутствие других optional metrics ограничивает
только зависящие от них решения и никогда не блокирует ручной `/next`.
...
- после `valid_until` current становится `EXPIRED`, затем начинается новый planning
  check-in; validity — 72 часа после `not_before`.
...
Initial overall weights для доступных областей: volume `0.35`, pace `0.20`, HR `0.10`,
readiness `0.25`, sleep `0.10`. Отсутствующий sleep исключается с renormalization и не
считается плохим восстановлением или недостатком пользователя: все sleep-поля полностью
optional. Stale sleep получает confidence `0` и не входит в readiness. Отсутствие HR
также не блокирует result, но ограничивает только HR-dependent решения. Confirmed manual
и user-confirmed AI preview имеют один typed contract; AI origin сам по себе не даёт
higher confidence.
...
fatigue/soreness/external load как `value / 10`. Initial numeric composition:
...
скрытым значением и не создаёт penalty. Sleep component использует доступные explicit
quality и/или duration как независимые сигналы из versioned config; duration не
записывается и не показывается как выдуманная «оценка качества сна».
...
Routing: `AiTask → configured provider/model → strict task adapter`. В MVP 1.0
единственный production-ready provider — OpenAI. Provider-neutral contracts, capability
checks и registry должны позволить позже добавить Gemini новым adapter/config без
изменения domain/application services, но nonfunctional Gemini stub и мёртвые env-поля не
добавляются. Другие vendors и generic OpenAI-compatible adapter остаются будущим scope.
OpenAI credentials переиспользуются всеми tasks, model выбирается per task deployment
config. Обычный Telegram user не выбирает vendor/model.

Extension seam покрывается contract test с in-memory fake provider под другим именем:
новый adapter регистрируется и получает поддерживаемую task без изменений domain,
application services или Telegram flow. Это критерий готовности инфраструктуры для
будущего Gemini, но не реализация Gemini API.
...
Принятое migration decision: переименовать `AssistedAccess`/table в общий
...
5. Selection: longest plausible session, завершившаяся за последние 36 часов; более
   старая получает stale status и не предлагается как «последняя ночь».
...
Все sleep data и permission полностью optional, включая duration, end time, source и
quality. Не каждый provider передаёт quality; backend не требует её и не выводит из
duration/stages. Поскольку companion sync остаётся ручным, бот не может разбудить телефон
и запросить свежий сон во время `/next`. Это должно быть явно указано в UX/known
limitations.
...
- прочитать зафиксированные решения раздела 18;
- добавить четыре ADR без повторного открытия согласованных defaults;
- создать implementation branch от свежего `main`, повторно проверить CI/Alembic head.

Gate: ADR committed before code, все продуктовые defaults однозначны.
...
- fake non-OpenAI provider регистрируется через public registry contract без изменений
  application service;
...
- полное отсутствие sleep не создаёт readiness penalty и не снижает доступность `/next`;
...
18. Единственный AI registry/task router обслуживает Activity, readiness и voice tasks;
    OpenAI — единственный real provider, а fake-provider contract доказывает расширяемость
    без vendor-specific веток в application service.
...
- native Gemini, generic OpenAI-compatible и другие provider adapters; MVP оставляет для
  них vendor-neutral registry boundary, но реализует только OpenAI;
...
Владелец подтвердил 12 июля 2026 года:

1. `general_fatigue` и `illness_symptoms` обязательны в confirmed check-in;
   `motivation` полностью optional.
2. Все sleep-поля и Health Connect sleep permission полностью optional. Sleep передаётся
   только при ручном companion sync; отсутствие duration/quality/source не блокирует и не
   ухудшает `/next`. Quality не синтезируется. Freshness window — 36 часов.
3. Consent повышается до v2. Existing owner ALLOWED/REVOKED переносится в общий external
   AI access, но пользователь должен заново принять расширенный consent; повторный owner
   request для уже ALLOWED не создаётся.
4. Единственный рабочий AI provider MVP 1.0 — OpenAI. Registry/task contracts остаются
   vendor-neutral и готовыми к отдельному Gemini adapter, но Gemini, generic
   OpenAI-compatible и другие adapters реализуются будущей итерацией.
5. Любые illness symptoms, movement-affecting/worsening pain или pain severity `>=6`
   возвращают REST. Recommendation valid 72 часа после `not_before`. Finish goal
   достигается с 98% target distance, time goal — только фактической Activity в коридоре
   target ±2%.

Эти решения не требуют повторного согласования следующего агента. Изменение любого из них
означает изменение scope/spec и новую запись в decision log.
