# Локальный запуск и deployment MVP 0.6
...
- `idaten-v0.6.1.apk` и `idaten-v0.6.1.apk.sha256` в GitHub Release;
- `ghcr.io/mattoyuzuru/idaten-backend:v0.6.1`;
...
git tag -a v0.6.1 -m 'Idaten v0.6.1'
...
```bash
mkdir -p /tmp/idaten-v0.6.1 && cd /tmp/idaten-v0.6.1
gh release download v0.6.1 -R MattoYuzuru/Idaten
shasum -a 256 --check idaten-v0.6.1.apk.sha256
"$ANDROID_HOME/build-tools/36.0.0/apksigner" verify --verbose idaten-v0.6.1.apk
docker buildx imagetools inspect ghcr.io/mattoyuzuru/idaten-backend:v0.6.1
```
...
DIGEST=$(docker buildx imagetools inspect \
  ghcr.io/mattoyuzuru/idaten-backend:v0.6.1 --format '{{json .Manifest.Digest}}' | tr -d '"')
...
6. Загрузить latest runs, отдельно разрешить/отклонить route, выполнить Manual Sync.
7. Убедиться, что duplicate не создает вторую Activity, а report остается private.
8. Отозвать device через `/revoke_device` и проверить отказ следующей sync.
