# Локальный запуск и deployment MVP 0.8
...
- `idaten-v0.8.0.apk` и `idaten-v0.8.0.apk.sha256` в GitHub Release;
- `ghcr.io/mattoyuzuru/idaten-backend:v0.8.0`;
...
git tag -a v0.8.0 -m 'Idaten v0.8.0'
...
```bash
mkdir -p /tmp/idaten-v0.8.0 && cd /tmp/idaten-v0.8.0
gh release download v0.8.0 -R MattoYuzuru/Idaten
shasum -a 256 --check idaten-v0.8.0.apk.sha256
"$ANDROID_HOME/build-tools/36.0.0/apksigner" verify --verbose idaten-v0.8.0.apk
docker buildx imagetools inspect ghcr.io/mattoyuzuru/idaten-backend:v0.8.0
```
...
DIGEST=$(docker buildx imagetools inspect \
  ghcr.io/mattoyuzuru/idaten-backend:v0.8.0 --format '{{json .Manifest.Digest}}' | tr -d '"')
...
Rich Messages Bot API 10.1 доступны в aiogram 3.29.1, но MVP 0.8 использует проверяемый
