# Локальный запуск и deployment MVP 0.9
...
Assisted text/screenshot input по умолчанию выключен. Для закрытой beta задайте
`BOT_OWNER_TELEGRAM_ID`, `ACTIVITY_EXTRACTION_ENABLED=true`,
`ACTIVITY_EXTRACTION_PROVIDER=OPENAI`, `ACTIVITY_EXTRACTION_MODEL` и
`ACTIVITY_EXTRACTION_API_KEY`. Endpoint, timeout/retry, text/image и daily/monthly limits
также настраиваются переменными из `.env.example`. Без key/provider manual, file import и
Health Connect работают как раньше. Live provider smoke выполняйте только на
owner-controlled test payload и не печатайте input или key.
...
- `idaten-v0.9.1.apk` и `idaten-v0.9.1.apk.sha256` в GitHub Release;
- `ghcr.io/mattoyuzuru/idaten-backend:v0.9.1`;
...
git tag -a v0.9.1 -m 'Idaten v0.9.1'
...
```bash
mkdir -p /tmp/idaten-v0.9.1 && cd /tmp/idaten-v0.9.1
gh release download v0.9.1 -R MattoYuzuru/Idaten
shasum -a 256 --check idaten-v0.9.1.apk.sha256
"$ANDROID_HOME/build-tools/36.0.0/apksigner" verify --verbose idaten-v0.9.1.apk
docker buildx imagetools inspect ghcr.io/mattoyuzuru/idaten-backend:v0.9.1
```
...
32 bytes), опционально `IMPORT_API_TOKEN` и `ACTIVITY_EXTRACTION_*`/owner variables из
...
или grants. Coach wording по умолчанию использует `LLM_PROVIDER=NONE`; extraction имеет
отдельные provider и enable settings.
...
DIGEST=$(docker buildx imagetools inspect \
  ghcr.io/mattoyuzuru/idaten-backend:v0.9.1 --format '{{json .Manifest.Digest}}' | tr -d '"')
...
16. Проверить `/next` и `/privacy`: дата/темп/причины читаемы, global/group кнопки
    обновляют один экран, forged/repeated callback не публикует Activity.

Rich Messages Bot API 10.1 доступны в aiogram 3.29.1, но MVP 0.9 использует проверяемый
