...
- `idaten-v0.9.0.apk` и `idaten-v0.9.0.apk.sha256` в GitHub Release;
- `ghcr.io/mattoyuzuru/idaten-backend:v0.9.0`;
...
git tag -a v0.9.0 -m 'Idaten v0.9.0'
...
```bash
mkdir -p /tmp/idaten-v0.9.0 && cd /tmp/idaten-v0.9.0
gh release download v0.9.0 -R MattoYuzuru/Idaten
shasum -a 256 --check idaten-v0.9.0.apk.sha256
"$ANDROID_HOME/build-tools/36.0.0/apksigner" verify --verbose idaten-v0.9.0.apk
docker buildx imagetools inspect ghcr.io/mattoyuzuru/idaten-backend:v0.9.0
```
...
DIGEST=$(docker buildx imagetools inspect \
  ghcr.io/mattoyuzuru/idaten-backend:v0.9.0 --format '{{json .Manifest.Digest}}' | tr -d '"')
