# Локальный запуск и deployment MVP 1.0
...
External AI input по умолчанию выключен. Для закрытой beta задайте
`BOT_OWNER_TELEGRAM_ID`, `AI_ENABLED=true`, `AI_OPENAI_API_KEY` и при необходимости
per-task provider/model из `.env.example`. Единственный production provider MVP 1.0 —
OpenAI; activity extraction, readiness extraction и voice transcription используют один
registry, endpoint, timeout/retry и общий daily/monthly budget. Без key/provider ручные
`/run`, `/next`, file import и Health Connect работают полностью. Live provider smoke
выполняйте только на owner-controlled payload и не печатайте input, transcript или key.

На workstation с TLS interception clean `docker compose build` может остановиться на
скачивании build dependency `hatchling` с `SSLCertVerificationError: self-signed
certificate in certificate chain`. Не отключайте certificate verification; в этом
случае локальные Compose up/migrate/health/ready не считаются пройденными, а clean image
build и runtime smoke должны быть подтверждены GitHub CI.
...
Базовая готовность требует только exercise/distance/HR/speed/elevation. Cadence читается,
если уже разрешён. `READ_SLEEP` запрашивается отдельной optional-кнопкой, отказ не меняет
состояние `READY`. Последний plausibility-valid sleep summary за 36 часов отправляется
только по явной кнопке «Синхронизировать сон»; stages/raw payload не передаются.
...
- `idaten-v1.0.0.apk` и `idaten-v1.0.0.apk.sha256` в GitHub Release;
- `ghcr.io/mattoyuzuru/idaten-backend:v1.0.0`;
...
git tag -a v1.0.0 -m 'Idaten v1.0.0'
...
```bash
mkdir -p /tmp/idaten-v1.0.0 && cd /tmp/idaten-v1.0.0
gh release download v1.0.0 -R MattoYuzuru/Idaten
shasum -a 256 --check idaten-v1.0.0.apk.sha256
"$ANDROID_HOME/build-tools/36.0.0/apksigner" verify --verbose idaten-v1.0.0.apk
docker buildx imagetools inspect ghcr.io/mattoyuzuru/idaten-backend:v1.0.0
```
...
32 bytes), опционально `IMPORT_API_TOKEN`, `AI_*` и `BOT_OWNER_TELEGRAM_ID` из
`.env.example`. Файл нельзя source-ить. Helper разбирает только allowlisted keys,
URL-encodes только password component `DATABASE_URL` и не выводит values:
...
или grants. При отсутствии `AI_ENABLED=true` и `AI_OPENAI_API_KEY` весь внешний AI path
выключен; отдельного coach wording provider больше нет.
...
DIGEST=$(docker buildx imagetools inspect \
  ghcr.io/mattoyuzuru/idaten-backend:v1.0.0 --format '{{json .Manifest.Digest}}' | tr -d '"')
...
Предыдущий подтвержденный healthy digest (`v0.7.0`) —
`sha256:7cc63d6b92509b8771b3e8b6e925735bab04d29fe27151eefe6cb5bbc04565cc`.
`v0.6.0` не является допустимой rollback target: он не достиг Ready из-за startup
blockers.
...
11. Проверить отказ от extraction consent без admin notification; затем consent,
    grant/revoke owner callback/command, text preview, Samsung Health screenshot, missing
    date edit, fuzzy duplicate и «Сохранить всё равно». Убедиться, что raw input не попал
    в storage/logs.
12. Отозвать device через `/revoke_device` и проверить отказ следующей sync.
13. Для text, photo и image document проверить немедленные «Распознаю пробежку…» и
    `typing`, один отредактированный preview, понятный timeout и отсутствие вечного статуса.
14. На iOS, Android и Desktop проверить `/stats`: восемь недель, числовые значения и
    читаемость Unicode-графика без зависимости от выравнивания.
15. Проверить `/pr` с 4,90/5,10 км, 9,80/10,20 км, полумарафоном и 11 км: оценка не
    должна называться фактическим рекордом.
16. Проверить manual-only `/next`: выбор/смену цели, полный POST_RUN preview, provisional,
    повтор до `not_before` без новой revision, изменение времени, idempotent «Пересчитать»
    и PRE_RUN unchanged/downgrade/postpone/REST.
17. Проверить external AI consent v2, existing owner allow после re-consent, readiness
    text/voice preview, revoke/timeout fallback и отсутствие raw text/image/audio/transcript
    в БД, storage и логах.
18. Отдельно отклонить `READ_SLEEP` и убедиться, что RUN sync и `/next` работают; затем
    разрешить его, вручную синхронизировать свежий сон, проверить provenance/edit/delete,
    stale >36 ч и отсутствие stages/raw payload.
19. Проверить `/privacy`: global/group кнопки обновляют один экран, forged/repeated
    callback не публикует Activity.

Rich Messages Bot API 10.1 доступны в aiogram 3.29.1, но MVP 1.0 использует проверяемый
