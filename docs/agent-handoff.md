...
Для MVP 1.0 подготовлен draft `docs/iterations/mvp-1.0.md`: adaptive deterministic
...
ещё не изменялись. До реализации владелец должен ответить на Q1–Q4 в конце спецификации,
после чего документ получает status `approved for implementation`, а новые решения
добавляются в decision log отдельными ADR.

- Planning branch: `codex/mvp-1.0-plan`.
- Следующий обязательный документ: `docs/iterations/mvp-1.0.md` после фиксации ответов.
