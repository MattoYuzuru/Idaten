...
Для MVP 1.0 одобрена спецификация `docs/iterations/mvp-1.0.md`: adaptive deterministic
`/next`, active running goal, typed readiness, provisional/pre-run lifecycle, unified AI
input infrastructure и optional Health Connect sleep prefill. Код, schema и ADR MVP 1.0
ещё не изменялись. Fatigue/illness обязательны, motivation и весь sleep slice optional,
consent v2 принят, рабочий provider — только OpenAI, safety/lifecycle thresholds
зафиксированы в разделе 18. Следующий агент сначала добавляет отдельные ADR из раздела 11,
затем выполняет slices по порядку без повторного продуктового согласования.

- Planning branch: `codex/mvp-1.0-plan`.
- Следующий обязательный документ: approved `docs/iterations/mvp-1.0.md`.
