...
MVP 0.6 развернут в production: Health Connect provider/permission onboarding,
постоянная release-подпись, GitHub Release/GHCR workflow и single-replica k3s deployment
за HTTPS. Coach, ingestion и privacy/source contracts не менялись. Остался только manual
acceptance на физическом Android-устройстве.
...
- Следующий обязательный документ: `docs/iterations/mvp-0.6.md`. Перед следующей
  итерацией завершить manual device checklist из `docs/deployment.md` и записать результат.
