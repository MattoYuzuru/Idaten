...
MVP 0.2 реализует Telegram groups, privacy settings, explicit per-activity opt-in,
membership, audited publications, weekly leaderboard и weekly streaks. Единственный
eligibility contract находится в backend group repository/service; Telegram handlers
остаются transport-слоем. Следующая итерация после merge MVP 0.2 начинает чтение с
`docs/iterations/mvp-0.3.md` и не должна расширять этот PR ее scope.
